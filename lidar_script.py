...
def edit_data(model):
    ...
    print("remove statistical outliers from the point cloud")
...
async def save_data_gltf(poisson_mesh):
...
def main():
    path = input("path to your ply-data")
    model = load_data(path)
    poisson_mesh, wireframe = edit_data(model)
    show_model(poisson_mesh, wireframe)
    save_data_gltf(poisson_mesh)
    asyncio.gather()
    print("your glTF file has been saved to '/gltf_3d_mash/reconstructed_mesh.gltf'")
